...
export default function Learning() {
  ...
  const [activeTab, setActiveTab] = useState('academy'); // 子分頁

  const learningTabs = [
    { id: 'academy', label: '登山知識' },
    { id: 'diagnostic', label: '登山診斷' },
    { id: 'selfcheck', label: '行前檢查' },
    { id: 'social', label: '社群分享' },
  ];
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="max-w-[1240px] mx-auto px-6 md:px-10 w-full">
        {/* 子分頁導覽 */}
        <div className="flex justify-center gap-8 md:gap-14 mt-14 mb-4 border-b border-border">
          {learningTabs.map(tab => (
            <span
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={cn(
                "pb-4 px-1 font-semibold text-sm md:text-base cursor-pointer transition-colors relative whitespace-nowrap",
                activeTab === tab.id ? "text-primary" : "text-text-muted hover:text-primary"
              )}
            >
              {tab.label}
              {activeTab === tab.id && (
                <motion.div layoutId="learning-tab-underline" className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary" />
              )}
            </span>
          ))}
        </div>
        ...
        <section className="py-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <Link to={`/explore?level=${encodeURIComponent('休閒級')}`} className="h-[420px] rounded-[20px] overflow-hidden relative cursor-pointer group block">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center px-6">
                ...
                <p className="text-sm tracking-widest opacity-90 mb-1">郊山介紹清單</p>
                <p className="text-xs opacity-0 group-hover:opacity-80 transition-opacity duration-500 tracking-widest">BEGINNER · CASUAL TRAILS</p>
              </div>
            </Link>
            <Link to={`/explore?level=${encodeURIComponent('挑戰級,專家級')}`} className="h-[420px] rounded-[20px] overflow-hidden relative cursor-pointer group block">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-white text-center px-6">
                ...
                <p className="text-sm tracking-widest opacity-90 mb-1">百岳・中級山縱走線</p>
                <p className="text-xs opacity-0 group-hover:opacity-80 transition-opacity duration-500 tracking-widest">ADVANCED · TRAVERSE ROUTES</p>
              </div>
            </Link>
          </div>
        </section>
        ...
        <section className="py-20 bg-white text-center -mx-6 md:-mx-10 px-6 md:px-10">
                ...
                >
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12 text-left">
                    ...
                    <div className="bg-primary text-white p-8 rounded-xl shadow-sm">
                      ...
                      <ul className="space-y-2 text-sm opacity-90">
                        {[
                          {
                            text: score >= 12 ? '維持每週兩次 10km 跑步與核心訓練' : '開始每週一次 5km 慢跑，建立基礎心肺',
                            url: 'https://hiking.biji.co/index.php?q=special&act=category_list&id=1',
                            label: '新手體能訓練專區'
                          },
                          {
                            text: score >= 8 ? '嘗試背負 10kg 進行郊山負重練習' : '先從輕量化裝備開始，避免膝蓋受傷',
                            url: 'https://hiking.biji.co/index.php?q=news&act=info&id=15174',
                            label: '如何挑選適合的登山背包'
                          },
                          {
                            text: answers[4]?.includes('GPX') ? '學習判讀等高線與指北針進階技巧' : '務必下載並學會使用「Hikingbook」或「Gaia GPS」',
                            url: answers[4]?.includes('GPX') ? 'https://hiking.biji.co/index.php?q=news&act=info&id=2740' : 'https://hikingbook.net/',
                            label: answers[4]?.includes('GPX') ? '圖針定位教學（健行筆記）' : 'Hikingbook 離線地圖 APP'
                          }
                        ].map((item, i) => (
                          <li key={i}>
                            <a
                              ...
                              className="flex items-start gap-3 px-3 py-2 -mx-3 rounded-md hover:bg-white/10 transition-colors group"
                            >
                              <CheckCircle2 size={18} className="text-accent flex-none mt-0.5" />
                              <div className="flex-1">
                                <p>{item.text}</p>
                                <p className="text-[0.7rem] text-white/50 mt-1 group-hover:text-accent transition-colors">→ {item.label}</p>
                              </div>
                            </a>
                          </li>
                        ))}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
                    {result.mountains.map((mt) => {
                      // 推薦山岳對應 routesData 的 id
                      const idMap: Record<string, string> = {
                        '中央尖山': '01', '玉山主峰': '02', '雪山主峰': '03',
                        '合歡北峰': '04', '大霸尖山': '05', '嘉明湖': '07',
                      };
                      const routeId = idMap[mt.name];
                      const linkTo = routeId ? `/route/${routeId}` : `/weather?q=${encodeURIComponent(mt.name)}`;
                      return (
                        <Link key={mt.name} to={linkTo} className="bg-white border border-border rounded-xl overflow-hidden group shadow-sm hover:shadow-md hover:border-accent transition-all block">
                          ...
                          <div className="p-8 text-left">
                            <div className="flex justify-between items-start">
                              <div>
                                ...
                                <p className="text-[0.7rem] text-text-muted mt-2 group-hover:text-accent transition-colors">
                                  → {routeId ? '查看完整攻略' : '查看即時氣象'}
                                </p>
                              </div>
                              <ChevronRight size={24} className="text-primary group-hover:text-accent group-hover:translate-x-1 transition-all" />
                            </div>
                          </div>
                        </Link>
                      );
        ...
        {activeTab === 'selfcheck' && (
        <section className="py-20 bg-[#E9F1F1] -mx-6 md:-mx-10 px-6 md:px-10 text-center">
        ...
        <section className="py-20 text-center relative">
          ...
          <div className="relative mt-20 group">
            ...
            >
              {[
                { title: '鳶嘴山：台中最美中級山的登山冒險日記', desc: '岩稜陡峭如刃，全程需手腳並用拉繩攀岩。沿途松林點綴，登頂視野遠眺中部平原...', date: '2025/08/11', author: '旅人日記', rating: 5, img: 'https://upload.wikimedia.org/wikipedia/commons/thumb/f/fa/Yuanzui_Mountain.jpg/1280px-Yuanzui_Mountain.jpg', url: 'https://rlifetw.pixnet.net/blog/post/198836450' },
                { title: '雪山主峰 & 雪山東峰：搭公車說走就走的入門新手級百岳', desc: '在黑森林的寂靜中前行，最終迎來圈谷的壯闊與主峰的日出，是台灣百岳的經典代表...', date: '2025/07/20', author: '健行筆記', rating: 5, img: 'https://upload.wikimedia.org/wikipedia/commons/5/5f/HsuehMountain.jpg', url: 'https://hiking.biji.co/index.php?q=review&act=info&review_id=23601' },
                { title: '谷關七雄之馬崙山：舖滿松針的五星級登山步道', desc: '海拔落差大，但整條步道鋪滿松針，腳感柔軟有彈性，是谷關七雄中最優美的路線...', date: '2025/04/01', author: 'Eddie', rating: 5, img: 'https://upload.wikimedia.org/wikipedia/commons/thumb/c/cb/%E5%85%AB%E4%BB%99%E5%B1%B1_Baxian_Mountain_-_panoramio.jpg/1280px-%E5%85%AB%E4%BB%99%E5%B1%B1_Baxian_Mountain_-_panoramio.jpg', url: 'https://eddiewu0718.pixnet.net/blog/post/226070669' },
                { title: '嘉明湖三天兩夜：天使的眼淚心得記錄分享', desc: '在稜線上看著雲海翻騰，最終見到那一池湛藍，所有的疲累都煙消雲散了...', date: '2025/03/15', author: 'Melissa 山女孩', rating: 5, img: 'https://upload.wikimedia.org/wikipedia/commons/thumb/9/94/Chiaming_Lake_20180718_01_%28cropped%29.jpg/1280px-Chiaming_Lake_20180718_01_%28cropped%29.jpg', url: 'https://melissalin510.pixnet.net/blog/post/323541700' },
                { title: '玉山主峰兩天一夜：海拔3952m加碼排雲山莊開箱', desc: '凌晨三點出發，在碎石坡上與風搏鬥，當太陽升起的那一刻，眼前的雲海美得令人屏息...', date: '2025/02/10', author: '呆呆齡愛玩樂', rating: 5, img: 'https://upload.wikimedia.org/wikipedia/commons/thumb/6/68/Yushan_main_east_peak%2BHuang_Chung_Yu%E9%BB%83%E4%B8%AD%E4%BD%91%2B9030.png/1280px-Yushan_main_east_peak%2BHuang_Chung_Yu%E9%BB%83%E4%B8%AD%E4%BD%91%2B9030.png', url: 'https://rowing2005.pixnet.net/blog/post/564189874' }
              ].map((post, i) => (
                <a 
                  key={i} 
                  href={post.url} 
